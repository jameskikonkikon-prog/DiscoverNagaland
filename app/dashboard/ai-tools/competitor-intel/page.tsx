    ...
    return (
      <div style={s.page}><div style={s.container}><div style={s.card}>
        <div style={s.titleRow}>
          ...
          <div><div style={s.title}>🔍 Competitor Intel</div><div style={s.sub}>No business linked to your account.</div></div>
        </div>
      </div></div></div>
    );
  }
      ...
      <div style={s.container}>
        <div style={s.card}>
          <div style={s.titleRow}>
            <Link href="/dashboard" style={s.back}>←</Link>
            <div>
              <div style={s.title}>🔍 Competitor Intel</div>
              <div style={s.sub}>AI analysis of your competition in Nagaland — understand your market, find your edge, and spot opportunities others are missing.</div>
            </div>
          </div>

          <div style={s.infoBox}>
            <div style={s.infoTitle}>{business.name}</div>
            <div style={s.infoSub}>The AI will analyse your business category, location, and the Nagaland market to give you a comprehensive competitive intelligence report.</div>
          </div>
          ...
          <div style={s.actions}>
            <button
              ...
              style={s.primaryBtn}
            >
              {aiLoading ? 'Analysing market…' : '🔍 Run Competitor Analysis'}
            </button>
          </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  infoBox: { background: '#0a0a0a', border: '1px solid #1e1e1e', borderRadius: 10, padding: 14, marginTop: 14 },
  infoTitle: { fontSize: 15, fontWeight: 700, marginBottom: 6 },
  infoSub: { fontSize: 12, color: '#888', lineHeight: 1.5 },
